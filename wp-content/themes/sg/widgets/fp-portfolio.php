...
class Sg_Portfolio extends WP_Widget {

	public function __construct() {
		$widget_ops = array('classname' => 'sg_portfolio_widget', 'description' => __( 'Display your projects in a grid.', 'sg') );
       parent::__construct(false, $name = __('SG FP: Portfolio', 'sg'), $widget_ops);
		$this->alt_option_name = 'sg_portfolio_widget';
  }
  ...
  public function widget( $args, $instance ) {
    ...
    $cache = array();
    if ( ! $this->is_preview() ) {
    	$cache = wp_cache_get( 'sg_portfolio', 'widget' );
    }
    ...
    if ($options['filter'] == 1) {
       ...
       foreach( $included_terms as $term ) {
           $term_obj = get_term_by( 'slug', $term, 'course_cat');
       ...
       $terms = get_terms( 'course_cat', array( 'include' => $id_string ) );
    ...
    while ( $the_query->have_posts() ):
       ...
       global $post;
       $id = $post->ID;
       $termsArray = get_the_terms( $id, 'course_cat' );
       $termsString = "";
       ...
       if ( $termsArray) {
           foreach ( $termsArray as $term ) {
			 ...
			 $project_title = '<div class="project-wrap">';
			 $project_title .= '<div class="project-title">';
			 $project_title .= '<h6 style="margin: 0px; padding: 10px">'.get_the_title($post->ID).'</h6>';
			 $project_title .= '</div>';
			 $project_title .= '</div>';
  ...
  function update($new_instance, $old_instance) {
    ...
    if ( isset($alloptions['sg_portfolio_widget']) )
    	delete_option('sg_portfolio_widget');
